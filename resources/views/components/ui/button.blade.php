...
@endphp
@if ($href)
    <a href="{{ $href }}" data-slot="button" {{ $attributes->class([$base, $variants[$variant], $sizes[$size]]) }}>{{ $slot }}</a>
@else
    <button type="{{ $type }}" data-slot="button" {{ $attributes->class([$base, $variants[$variant], $sizes[$size]]) }}>{{ $slot }}</button>
@endif
