...
- **Layouts:** `layouts/blank` (auth pages), `layouts/main` (public site, top navbar), `layouts/user` (signed-in area: dashboard, notes, account; Next's inset sidebar shell, `data-layout="user"`), all PJAX-aware; admin mirrors them in `modules/admin/layouts/`.
